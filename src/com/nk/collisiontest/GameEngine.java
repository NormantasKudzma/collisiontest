...
//	
//	public void setBitmapData(int [] arr){
//		bitmapData = arr;
//	}
...
//		float delta;
//		if (left){
//			delta = Math.min(raycastHorizontal(x, y + 1, true), raycastHorizontal(x, y + h - 1, true));
//		}
//		else {
//			delta = Math.min(raycastHorizontal(x + w + 1, y, false), raycastHorizontal(x+w, y + h - 1, false));
//		}

		float movementValue = val * horizontalSpeed;
//		delta = Math.min(delta, Math.abs(movementValue));

//		x += delta * Math.signum(val);
		x += movementValue;
		...
		if (raycastHorizontal(x, y + 1, true) == 0 || raycastHorizontal(x, y + h - 1, true) == 0 ||
				raycastHorizontal(x + w + 1, y, false) == 0 || raycastHorizontal(x+w, y + h - 1, false) == 0){
...
//			Log.w("nk", "RAYCAST UP DIST : " + delta + " accel : " + acceleration);
...
//			if (bitmapData[xPos + screenWidth * yPos] != wall){
				result++;
				xPos += delta;
			}
			else {
	...
	public int raycastVertical(int xPos, int yPos, boolean up){
		...
		gameView.paint.setColor(Color.GREEN);
		while (yPos > 0 && yPos < screenHeight){
			if (bitmap.getPixel(xPos, yPos) != wall){
				gameView.mainCanvas.drawPoint(xPos, yPos, gameView.paint);
				result++;
				yPos += delta;
			}
			else {
				gameView.paint.setColor(Color.RED);
				gameView.mainCanvas.drawRect(xPos - 1,  yPos - 1, xPos + 1, yPos + 1, gameView.paint);
				break;
			}
		}
	...
	public void loadLevel(Resources res){
		try {
			BufferedReader br = new BufferedReader(new InputStreamReader(res.getAssets().open("level" + currentLevel)));
			String line;
